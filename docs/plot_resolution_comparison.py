...
plt.suptitle(
    "GMRT Resolution Comparison: Mid-Atlantic Ridge", fontsize=16, y=1.02
)
plt.tight_layout()
plt.show()
...
for res, size, time_est, quality in zip(
    resolutions, file_sizes, download_times, detail_scores
):
...
bars1 = ax1.bar(
    resolutions, file_sizes, color=["#d62728", "#ff7f0e", "#2ca02c"]
)
...
for i, res in enumerate(resolutions):
    ax2.annotate(
        res.title(),
        (download_times[i], detail_scores[i]),
        xytext=(5, 5),
